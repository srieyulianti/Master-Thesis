...
static sgx_status_t SGX_CDECL sgx_ecall_get_target_info(void* pms)
{
	...
	ms->ms_retval = ecall_get_target_info(_in_target_info);
	if (_in_target_info) {
		if (memcpy_s(_tmp_target_info, _len_target_info, _in_target_info, _len_target_info)) {
...
static sgx_status_t SGX_CDECL sgx_sgx_tvl_verify_qve_report_and_identity(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	const uint8_t* _tmp_p_quote = ms->ms_p_quote;
	uint32_t _tmp_quote_size = ms->ms_quote_size;
	size_t _len_p_quote = _tmp_quote_size;
	uint8_t* _in_p_quote = NULL;
	const sgx_ql_qe_report_info_t* _tmp_p_qve_report_info = ms->ms_p_qve_report_info;
	size_t _len_p_qve_report_info = 1 * sizeof(sgx_ql_qe_report_info_t);
	sgx_ql_qe_report_info_t* _in_p_qve_report_info = NULL;
	const uint8_t* _tmp_p_supplemental_data = ms->ms_p_supplemental_data;
	uint32_t _tmp_supplemental_data_size = ms->ms_supplemental_data_size;
	...
	ms->ms_retval = sgx_tvl_verify_qve_report_and_identity((const uint8_t*)_in_p_quote, _tmp_quote_size, (const sgx_ql_qe_report_info_t*)_in_p_qve_report_info, ms->ms_expiration_check_date, ms->ms_collateral_expiration_status, ms->ms_quote_verification_result, (const uint8_t*)_in_p_supplemental_data, _tmp_supplemental_data_size, ms->ms_qve_isvsvn_threshold);
