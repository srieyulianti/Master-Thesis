...
#define CERT_SUBJECT_NAME_VALUES  "CN=RATLS,O=OcclumDevelopers,C=US"
...
static int create_x509(mbedtls_pk_context* pk, mbedtls_x509write_cert* writecrt) {
    ...
    int ret = sha256_over_pk(pk, report_data.d);
    if (ret < 0)
        return ret;
    ...
    ret = generate_x509(pk, quote_buffer, quote_size, writecrt);
