...
int main()
{
	...
	/* Define hostname for remote connection */
	...
	if (wolfSSL_CTX_use_certificate_buffer(ctx, crt, crt, crt_len, SSL_FILETYPE_ASN1)
        != SSL_SUCCESS) {
        fprintf(stderr, "ERROR: failed to load %s, please check the file.\n",
                crt);
        return -1;
	...
	/* Continue to accept clients until shutdown is issued */
	 while (!shutdown) {
		...
		printf(stdout, "[+] Client connected successfully\n");
